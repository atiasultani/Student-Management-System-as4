...
for (let record =1 ; record<= 3 ; record++) {
  ...
  console.log("--------------------------------------");
  console.log("\n");
  let studend_rollNo: number = Math.floor(10000 + Math.random() * 50000);
  console.log(`Student Roll No : ` + studend_rollNo);
  console.log("\n");
  let student_Name: string = await inquirer.prompt([
    {
      name: "student",
      type: "input",
  ...
  let studentCourse: any = await inquirer.prompt([
    {
      name: "courseEnroll",
      type: "list",
  ...
  let medicalTotalFee: number =
    medicalFee.tuition + medicalFee.admission + medicalFee.annual;
  let computerFee = { tuition: 15000, admission: 5000, annual: 15000 };
  let computerTotalFee: number =
    computerFee.tuition + computerFee.admission + computerFee.annual;
  let fine_ArtsFee = { tuition: 5000, admission: 5000, annual: 5000 };
  let fine_ArtsTotalFee: number =
    fine_ArtsFee.tuition + fine_ArtsFee.admission + fine_ArtsFee.annual;
  ...
  if (studentCourse.courseEnroll == "Medical") {
    ...
    let paymentPay: any = await inquirer.prompt([
